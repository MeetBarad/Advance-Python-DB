...
def inite():
    ...
    cursor=cnn.cursor()
    cursor.execute("create table IF NOT EXISTS data(id integer primary key autoincrement,name text,price integer)")
    cnn.commit()
    cursor.close()
    cnn.close()
inite()
...
def insert_recorde():
    ...
    if(nm=="" or pr==""):
        ...
        cnn=sq.connect("my.db")
        cursor=cnn.cursor()
        cursor.execute("insert into data(name,price)values(?,?)",(nm,pr))
...
def search_record():
    ...
    cnn=sq.connect("my.db")
    cursor=cnn.cursor()
    cursor.execute("SELECT * FROM data where name LIKE ? ",(f"%{txt3.get()}%",))
...
def Edit_record():
    ...
    if(get_id["values"]==""):
        ...
        def update_record():
            ...
            cnn=sq.connect("my.db")
            cursor=cnn.cursor()
            cursor.execute("update data set name=?,price=? where id=?",(nm1,pr1,row_id))
